...
The default remote (`cloudflare_r2_nemo`) is a public Cloudflare R2 bucket - no
credentials needed to pull. Pushing requires the private S3 remote
(`cloudflare_r2_nemo_push`), configured locally in `.dvc/config.local` (gitignored),
with R2 credentials passed via env vars.
